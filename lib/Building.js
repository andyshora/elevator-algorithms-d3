  ...
  _createClass(Building, [{
    ...
    value: function workElevators(t) {
      ...
      for (var i = 0; i < this._numFloors; i++) {
        ...
        _Utils.Utils.log('There are ' + people.length + ' people waiting on floor ' + i, people);

        // if there's people waiting on this floor
        if (people.length) {
          var elevators = this.getElevatorsWaitingOnFloor(i);
          _Utils.Utils.log('There are ' + elevators.length + ' elevators waiting on floor ' + i, elevators);
          for (var j = 0; j < elevators.length; j++) {
            ...
            var p = undefined;
            _Utils.Utils.log('Unloading elevator ' + j + ' at floor ' + i);

            while (p = elevators[j].unloadPerson()) {
              _Utils.Utils.log('   ' + p.name + ' (id:' + p.id + ')');
              p.updateState(i, 'resting');
            }

            // load people into elevators
            var k = 0;
            _Utils.Utils.log('Loading elevator ' + j + ' at floor ' + i);

            while (elevators[j].hasRoom() && k < people.length) {
              _Utils.Utils.log('   ' + people[k].name + ' (id:' + people[k].id + ')');
